...
user_tweet <- function(user, maxtweets = 100, home = FALSE, parse = TRUE, check = TRUE, token = NULL, 
  ...
  if(!(output_file_name %in% list.files())){
    tuits <- get_timeline(user = user, n = maxtweets, home = home, parse = parse, check = check, token = token, include_rts = include_rts)
    if (format[1] == 'binary') save( tuits, file=output_file_name)
    else  if (format[1] == "delimited") write_as_csv(x = tuits, file_name = output_file_name)
    print(paste("Numero de tweets nuevos:", nrow(tuits)))
...
search_tweet <- function(search, maxtweets = 300, type = "recent", include_rts = TRUE, geocode = NULL, since_id = NULL, 
  ...
  if(!(output_file_name %in% list.files())) {
    tuits <- search_tweets(search, n = maxtweets, type = type, include_rts = include_rts, geocode = geocode, since_id = since_id, parse = parse, token = token, retryonratelimit = retryonratelimit, verbose = verbose)
...
stream_tweet <- function(search, timeout = 120, parse = TRUE, token = NULL, output_file_name = search, verbose = TRUE, dir = NULL, format = c("binary", "delimited")) {
    ...
    tuits <- parse_stream(output_file_name_json)
    ...
    print(paste("Numero de tweets:", nrow(tuits)))
