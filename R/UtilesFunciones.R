...
tTweetCoin <- function(data, text="text", searchers="@#", original= TRUE, excludeRT=TRUE, min=1, others=NULL, oldText=NULL, newText=NULL, ...) {
  arguments <- list(...)
  X <- data[[text]]
...
retCoin <-function(Tuits, author="author", text="text", date="date",
  ...
  TT <-
    aggregate(target~author, FUN = length, data=Tuits[Tuits$target=="",])
  names(TT) <- c("name","Tweets")
  
  Messages <-
    Tuits [Tuits$target=="" & Tuits$author!="",]
  Messages <- Messages[order(Messages$date),]
  Messages$location <- substr(Messages$location,1,50)
  ...
  for(i in names(serie)) {
    ...
    tt <- # Original number of tweets by author
    ...
    nodes <- nodes[,c("name","link", "tweets", "retweets", "retweeted",
                      ...
                      "followers", "following","stauses", 
                      "Walktrap", "walktrap", "location", "size", "label")]
